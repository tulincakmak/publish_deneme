# -*- coding: utf-8 -*-
"""
Created on Thu Jan 11 10:56:24 2018
...
#    df['ppl_in_cabin'] = dataset.groupby(['Cabin'])['Cabin'].transform('count').fillna(0)
#    df['Deck'] = dataset.Cabin.str.extract('([A-Z])', expand=True)[0].fillna('DeckNan')
#    df =pd.get_dummies(dataset['Deck'], dummy_na=True)
...
x_train, y_train, x_test, y_test = split_data(train, 0.20, LABEL)
...
for train, test in kfold.split(x_train, y_train):
    ...
    history=classifier.fit(x_train, y_train, batch_size = 25, epochs = 500, validation_data=(x_test, y_test), validation_split=0.2)
    ev=classifier.evaluate(x_test,y_test)
    print("%s: %.2f%%" % (classifier.metrics_names[1], ev[1]*100))
    cvscores.append(ev[1] * 100)
...
#dataset.to_csv(r"C:\Users\tulincakmak\Desktop\deneme12.csv")